...
- Automated test coverage must maintain **>= 95%** statement coverage of the production code under `scripts/`, measured by the offline test suites.
...
| **Function Length** | `tests/test_principles.py` (AST line count) | <= 60 lines per function, in `tests/` and `scripts/` |
...
| **Test Coverage** | `make coverage` (`pytest --cov=scripts`) | >= 95% of production code (`fail_under`) |
